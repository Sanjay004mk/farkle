...
@onready var area_3d: Area3D = $Path3D/PathFollow3D/Node3D/MeshInstance3D/Area3D
...
func _orient_die():
	var data = _DIE_NUMBER_FACES_AND_ANGLES[number - 1]
	mesh_instance_3d.rotate(data[0], deg_to_rad(data[1]))
...
func _reset_die_transform():
	...
	mesh_instance_3d.rotation = Vector3.ZERO
...
func roll(p_distribution: float) -> int:
	...
	mesh_instance_3d.rotate(Vector3.UP, deg_to_rad(randf_range(-10, 10)))
